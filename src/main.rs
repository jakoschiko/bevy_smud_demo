...
fn background(global_state: Res<GlobalState>, mut clear_color: ResMut<ClearColor>) {
    clear_color.0 = convert_color(global_state.background_color);
}
                ...
                egui::ScrollArea::horizontal()
                    ...
                    .show(ui, |ui| {
                        for id in shapes {
                            ...
                            if global_state.scroll_to == Some(id) {
                                global_state.scroll_to = None;
...
fn update_shape(
    ...
    *transform = Transform::from_translation(shape_state.position)
    ...
    if compile_shader {
        let sdf_shader_code = add_unique_shader_import_path(&shape_state.sdf_code, global_state);
        let sdf_shader = Shader::from_wgsl(sdf_shader_code, file!());
        shape.sdf = shaders.add(sdf_shader);

        let fill_shader_code = add_unique_shader_import_path(&shape_state.fill_code, global_state);
